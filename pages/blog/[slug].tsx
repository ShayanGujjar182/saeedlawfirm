...
export default function BlogArticleRoute({ page, html }: Props) {
  return <DynamicSeoPage page={page} html={html} />
}

export const getStaticPaths: GetStaticPaths = () => ({
  paths: listSeoPagesByKind('article').map(page => ({ params: { slug: page.slug } })),
  fallback: false,
})

export const getStaticProps: GetStaticProps<Props> = ({ params }) => {
  const slug = String(params?.slug ?? '')
  const page = findSeoPageByRoute(`/blog/${slug}`)
